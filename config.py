...
# Нарезать свой текст на шоты по РЕАЛЬНОМУ звучанию, а не по числу слов.
# Тогда WORDS_PER_SEC вообще не влияет на результат: сначала синтезируется вся
# озвучка, потом по её таймкодам ставятся границы шотов, и шот получается
# нужной длины независимо от того, насколько быстро говорит голос.
# Требует включённой озвучки; без неё автоматически падает на счёт по словам.
SPLIT_BY_TIMING     = _b("SPLIT_BY_TIMING", True)
...
# Оформление плашки:
#   outline — белый текст с чёрной обводкой (как в референсе ИСТОКа)
#   glow    — обводка потоньше плюс мягкая тень: легче читается на пёстром кадре,
#             при этом не выглядит «тяжёлым»
#   box     — текст в полупрозрачной подложке. Самый читаемый вариант на любом
#             фоне, потому что не зависит от того, что под ним; но занимает
#             больше места и заметнее закрывает картинку.
CAPTION_STYLE      = _opt("CAPTION_STYLE", "glow")
CAPTION_BOX_ALPHA  = _i("CAPTION_BOX_ALPHA", 0xA0)   # прозрачность подложки box
...
# Как подаётся промо-плашка. Крупный баннер поверх кадра площадки распознают
# как рекламную вставку и режут охват, поэтому есть более сдержанные варианты:
#   banner — крупный текст сверху (как было; максимально заметно, но и риск выше)
#   native — мелкая полупрозрачная подпись внизу кадра, читается как элемент
#            оформления ролика, а не как вставленный баннер
#   corner — компактная подпись в углу, ещё незаметнее
PROMO_STYLE      = _opt("PROMO_STYLE", "native")
PROMO_OPACITY    = _f("PROMO_OPACITY", 0.85)         # 1.0 непрозрачно, 0.0 невидимо
