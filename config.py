...
# Гибрид «ведущая + история»: первые N шотов героиня обращается к зрителю
# (интро), дальше идёт обычная визуализация истории с персонажами в действии.
#
# Зачем именно так: липсинка нет и не будет — озвучка синтезируется отдельно от
# видео, и говорящий рот всегда расходится со звуком. Чем дольше герой говорит
# в кадре, тем заметнее рассинхрон. Короткое интро зритель прощает, минуту
# говорящей головы — нет. Поэтому ведущая только открывает ролик.
INTRO_HOST_SHOTS    = _i("INTRO_HOST_SHOTS", 0)      # 0 = выключено

# Запрет артикуляции во всех шотах. Липсинка нет: озвучка синтезируется
# отдельно от видео, поэтому любой говорящий рот в кадре расходится со звуком
# и сразу выдаёт подделку. Дешевле не показывать речь вообще — история
# рассказывается закадровым голосом, а персонажи ДЕЙСТВУЮТ.
NO_SPEECH           = _b("NO_SPEECH", True)

# Режим «проводник»: постоянная героиня не заменяет собой историю, а водит
# зрителя по ней. В части шотов её вообще нет — там действуют персонажи самой
# истории; там, где есть, она стоит сбоку и меньше по масштабу, а событие
# происходит вокруг неё.
#
# Зачем: у конвейера один референс персонажа, и без этого он подставляет
# героиню в КАЖДЫЙ кадр. История про мужчину девятнадцатого века превращается
# в серию портретов девушки, а самого действия не видно.
GUIDE_MODE  = _b("GUIDE_MODE", False)
GUIDE_RATIO = _f("GUIDE_RATIO", 0.4)   # доля шотов, где проводник в кадре

# Энергия движения в кадре. Модели по умолчанию экономят движение, а пресеты
# раньше сами просили «slow» и «almost imperceptible» — отсюда вялая картинка.
#   calm   — медленный дрейф (для спокойных сюжетов)
#   normal — уверенное движение камеры и героя
#   high   — быстрый, резкий монтажный ритм, как у вирусных каналов
MOTION_ENERGY = _opt("MOTION_ENERGY", "high")

# Как подгонять клип видеомодели под длину шота.
# Клип всегда заказывается с округлением ВВЕРХ (4с под шот в 3.2с), поэтому:
#   trim  — обрезать хвост: часть движения просто выбрасывается
#   speed — сжать по времени: то же движение укладывается в шот и идёт быстрее.
